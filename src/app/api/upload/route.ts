...
export async function POST(request: Request) {
  ...
  try {
    ...
    const diskHint =
      code === "EACCES" || code === "EROFS"
        ? " Server cannot write to public/uploads (check filesystem permissions or Docker image ownership)."
        : "";
    return NextResponse.json(
      { error: `Could not save upload.${diskHint}` },
      { status: 500 },
    );
  }
}
